...
class Parameter(object):
    ...
    def getValue(self, default):
        ...
        if default is Parameter.NO_VALUE or self.use_default == False:
            ...
            if default is not Parameter.NO_VALUE and v == '':
                v = default
        else:
...
def pip(c, pythonCmd=None):
    ...
    if pythonCmd is None:
        pythonCmd = CmdContextManager.python_cmd
...
class TaskCall(Argument):
    ...
    def execute(self):
        if self.name in tasks:
            ...
            raise self.TaskNotFound(self.name)
    ...
    while i < len(args):
        ...
        if arg.startswith('-'):
            ...
            if arg == '-V':
                ...
                if '=' not in args[i + 1]:
                    raise Exception('Option ("{}") must be in format name=value'.format(args[i + 1]))
            ...
            if not pv.requirements_exists() or pv.is_activated():
                ...
                try:
                    ...
                    if isinstance(e, ImportError) and (
                            getattr(e, 'message', '') == 'No module named pie_tasks' or getattr(e, 'msg', '') == "No module named 'pie_tasks'"):
